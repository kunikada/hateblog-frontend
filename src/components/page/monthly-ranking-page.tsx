...
const MONTHLY_RANKING_LIMIT = 1000

export function MonthlyRankingPage({ title, year, month, prev, next }: MonthlyRankingPageProps) {
  console.debug('[MonthlyRankingPage] Component mounted', { title, year, month })
  ...
  const { data, isLoading, error } = useQuery(
    rankingsQueryOptions.monthly({ year, month, limit: MONTHLY_RANKING_LIMIT }),
  )
  ...
  // Reset displayed count when year/month changes
  // biome-ignore lint/correctness/useExhaustiveDependencies: intentionally reset on year/month change
  useEffect(() => {
    setDisplayedCount(entriesPerPage)
  }, [year, month, entriesPerPage])
