...
const MONTHLY_RANKING_LIMIT = 100
...
export function MonthlyRankingPage({ title, year, month, prev, next }: MonthlyRankingPageProps) {
  ...
  const { displayedEntries, isLoading, isLoadingMore, hasMore, error, loadMoreRef } =
    useApiInfiniteScroll({
      queryKey: rankingsQueryOptions.keys.monthly({ year, month, limit: MONTHLY_RANKING_LIMIT }),
      queryFn: async ({ pageParam = 0 }) => {
        const result = await rankingsRepository.getMonthlyRanking({
          year,
          month,
      ...
      apiLimit: MONTHLY_RANKING_LIMIT,
      resetDeps: [year, month],
