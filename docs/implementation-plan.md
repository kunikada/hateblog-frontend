...
### 1.2 スコープ
- 10画面の実装（トップページ、エントリー一覧、ランキング、アーカイブ、検索、履歴）
...
- [ ] Button, Tabs, Dialog, Sheet, Badge, Calendar
- [ ] デザインシステム統一（Hatebuブルー基調）
- [ ] ダークモード切り替え機能（`ThemeProvider`）

#### A-1.2 レイアウトコンポーネント
- [ ] `AppLayout`（ヘッダー、フッター、グローバルナビゲーション）
  - モックの検索フォーム
  - ダークモード切り替えボタン
- [ ] `PageHero`（ページタイトル、説明）
- [ ] `EntryListSection`（エントリーリストコンテナ）
- [ ] レスポンシブ対応（PC/タブレット/スマホ）
...
- [ ] `EntryCard`（エントリーカード）
  ...
  - モックデータで表示確認
- [ ] `FilterBar`（はてブ数フィルタチップ）
- [ ] `Pagination`（ページネーション）
- [ ] `SkeletonList`（ローディング表示）
- [ ] `DatePicker`（日付選択カレンダー）
- [ ] `RankingBadge`（順位バッジ）

#### A-1.4 モックデータ作成
- [ ] `src/mocks/entries.ts`（エントリー一覧のモック）
- [ ] `src/mocks/rankings.ts`（ランキングのモック）
- [ ] `src/mocks/archive.ts`（アーカイブのモック）
...
- [ ] ルート定義（`/`, `/entries/:date/hot`）
- [ ] ページヒーロー
- [ ] フィルタバー（はてブ数閾値）UI
- [ ] 日付選択UI（カレンダー、前日/翌日ボタン）
- [ ] エントリーカード一覧（モックデータ表示）
- [ ] ページネーション UI
- [ ] レスポンシブ対応確認

#### A-2.2 新着順エントリー一覧
- [ ] ルート定義（`/entries/:date/new`）
- [ ] 人気順とのUI差分実装
- [ ] モックデータ表示
- [ ] レスポンシブ対応確認

**成果物**: `/`, `/entries/:date/hot`, `/entries/:date/new` 画面UI
...
#### A-3.1 年次ランキング
- [ ] ルート定義（`/rankings/:year`）
- [ ] ヒーローセクション（TOP3 大きく表示）
- [ ] ランキング一覧（4位以降、順位バッジ付き）
- [ ] 月次ランキングへのリンク UI
- [ ] モックデータ表示

#### A-3.2 月次ランキング
- [ ] ルート定義（`/rankings/:year/:month`）
- [ ] 月選択 UI
- [ ] モックデータ表示

#### A-3.3 週次ランキング
- [ ] ルート定義（`/rankings/:year/week/:week`）
- [ ] 週選択 UI
- [ ] モックデータ表示

**成果物**: `/rankings/:year`, `/rankings/:year/:month`, `/rankings/:year/week/:week` 画面UI
...
**成果物**: `/archive`, `/tags/:tag`, `/search/:search`, `/history` 画面UI
...
1. shadcn/ui コンポーネント統合
   Button, Badge, Dialog, Sheet, Tabs, Calendar
   ↓
2. レイアウトコンポーネント
   AppLayout, PageHero, EntryListSection
   ↓
3. 共通UIコンポーネント
   EntryCard, FilterBar, Pagination, DatePicker, RankingBadge
   ↓
4. モックデータ作成
...
| MA-1: 基盤UI | 3-4日 | 共通コンポーネント、モックデータ | コンポーネントカタログ（モックデータ表示） |
| MA-2: エントリーUI | 3-4日 | トップページ、エントリー一覧UI | **エントリー一覧画面（モックデータ）** |
| MA-3: ランキングUI | 2-3日 | ランキング画面UI | **ランキング画面（モックデータ）** |
| MA-4: その他UI | 3-4日 | アーカイブ、検索、履歴UI | **全10画面UI（モックデータ）** |
