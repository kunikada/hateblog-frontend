...
#### A-2.2 日付指定の人気順エントリー一覧
- [x] ルート定義（`/entries/$date/hot`）
- [x] 人気順と同じレイアウト
- [x] DateNavigation（前日/翌日リンク）の表示
- [x] モックデータ表示
- [x] レスポンシブ対応確認

#### A-2.3 日付指定の新着順エントリー一覧
- [x] ルート定義（`/entries/$date/new`）
- [x] 人気順と同じレイアウト
- [x] DateNavigation（前日/翌日リンク）の表示
- [x] モックデータ表示（新着順ソート）
- [x] レスポンシブ対応確認
...
- `DateNavigation` - 日付ナビゲーション（前日/翌日リンク、hot/new対応）
...
#### A-3.1 週次ランキング
- [x] ルート定義（`/rankings/weekly`）
- [x] ページタイトル（メインコンテンツ内）
- [x] TOP3大きく表示（RankingBadge: 金銀銅）
- [x] ランキング一覧（4位以降、RankingBadge付き）
- [x] 週選択UI（前週/次週リンク）
- [x] サイドバー（人気のタグ、他期間ランキングリンク）
- [x] モックデータ表示

#### A-3.2 月次ランキング
- [x] ルート定義（`/rankings/monthly`）
- [x] 週次と同じレイアウト
- [x] 月選択UI（前月/次月リンク）
- [x] モックデータ表示

#### A-3.3 年次ランキング
- [x] ルート定義（`/rankings/yearly`）
- [x] 年選択UI（前年/次年リンク）
- [x] モックデータ表示
...
- [x] ルート定義（`/archive`）
- [x] ページタイトル（メインコンテンツ内）
- [x] 全日付リスト表示（Accordion使用）
  - 年ごとにセクション分け
  - 月ごとに展開/折りたたみ
  - 日付リンク（エントリー件数表示）
- [x] 日付クリックで人気エントリー画面（`/entries/$date/hot`）へ遷移
- [x] モックデータ表示
- [x] レスポンシブ対応確認
...
- [x] Infinite scroll（IntersectionObserver使用）
- [x] フィルタバー（はてブ数閾値）
- [x] サイドバー（人気のタグ、週間ランキング）
- [x] モックデータ表示

#### A-4.3 検索結果
- [x] ルート定義（`/search`）
- [x] 検索フォーム（再検索用、Search iconあり）
- [x] エントリーカード一覧
- [x] キーワードハイライト（複数キーワード対応）
- [x] Infinite scroll（IntersectionObserver使用）
- [x] フィルタバー（はてブ数閾値）
- [x] サイドバー（人気のタグ、週間ランキング）
- [x] モックデータ表示
...
#### A-4.4 閲覧履歴
- [x] ルート定義（`/history/$date`）- 日付単位で表示
- [x] ページタイトル（日付表示）
- [x] DateNavigation（前日/翌日リンク）
- [x] エントリーカード一覧（閲覧時刻表示：HH:mm）
- [x] Infinite scroll（IntersectionObserver使用）
- [x] 履歴削除ボタン（個別×ボタン）
- [x] サイドバー（人気のタグ、週間ランキング）
- [x] モックデータ表示（localStorage想定）

**成果物**: `/archive`, `/tags/$tag`, `/search`, `/history/$date` 画面UI ✅

**追加で作成したコンポーネント**:
- shadcn/ui `Accordion` コンポーネントを追加（アーカイブ用）
...
| MA-3: ランキングUI ✅ | 2-3日 | 週次/月次/年次ランキング画面UI | **ランキング画面（モックデータ）** |
