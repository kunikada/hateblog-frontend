...
- [ ] ルート定義（`/`）
- [ ] メインコンテンツレイアウト（メインカラム + サイドバー）
- [ ] ページタイトル（メインコンテンツ内に直接配置）
- [ ] フィルタバー（はてブ数閾値: 全て、5+、10+、50+、100+、500+、1000+ users）
- ~~日付選択UI~~（不要 - DateNavigationは将来的にアーカイブから遷移時に使用）
- [ ] エントリーカード一覧（モックデータ表示）
- [ ] Infinite scroll（IntersectionObserver使用）
- [ ] ローディング表示（SkeletonList）
- [ ] サイドバー（人気のタグ、週間ランキング、PC表示のみ）
- [ ] トップに戻るボタン（スクロール300px以降表示）
- [ ] レスポンシブ対応確認
